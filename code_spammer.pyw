...
def sign_up_redeem():
    ...
    for _ in range(num_accounts):
        ...
        try:
            ...
            log(f"Response: {resp}")
            log(f"Waiting {wait_time} Seconds due to api limits")
...
def redeem_promo_code(auth_token, promo_code):
    url = "https://wildbet.gg/api/promo-code/redeem-code"
...
# GUI setup
root = tk.Tk()
root.title("WildBet.gg Code Spammer")
...
label_promo_code = tk.Label(frame, text="Robux Code:")
label_promo_code.grid(row=2, column=0, padx=5, pady=5, sticky="w")

label_wait_code = tk.Label(frame, text=" Wait Time:")
label_wait_code.grid(row=3, column=0, padx=5, pady=5, sticky="w")

entry_promo_code = tk.Entry(frame)
entry_promo_code.grid(row=3, column=1, padx=5, pady=5)
...
text_logs.grid(row=4, column=2, rowspan=4, padx=10, pady=10)
